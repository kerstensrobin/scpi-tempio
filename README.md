...
> [!WARNING]
> **Hardware v0.01 is not ready for production.** The board must be revised before another batch is ordered. Open issues:
>
> 1. **J1 VTref is on VBUS (5 V) on the PCB.** The schematic was corrected (VTref to 3V3), but the PCB was never updated from it. With 5 V on VTref, a programmer that sets its I/O level from VTref (such as a genuine ST-Link) may drive SWD at 5 V into a 3.3 V MCU. Existing boards need a bodge: cut J1 pin 1 from VBUS and wire it to 3V3.
> 2. **The schematic hotfix also moved C7** (1 µF, the AP2112K input capacitor) from VBUS to 3V3. It must go back on VBUS before the PCB is updated from the schematic, or the regulator loses its input capacitor.
> 3. **U1 symbol still named STM32G070KBTx** in the schematic's symbol cache; the fitted part is the STM32G0B1KBT6.
> 4. **Gerbers in `hardware/production/` are from v0.01** and contain issue 1. Regenerate them after the fixes.
>
> Before ordering: fix the above, update the PCB from the schematic, run DRC with schematic parity, and check that J1 pin 1 is on 3V3 and C7 on VBUS.
...
| Path          | Contents                                                        |
| ------------- | --------------------------------------------------------------- |
| `hardware/`   | KiCad project `SCPI_tempIO` (board with rounded tracks), `production/` holds the v0.01 JLC outputs |
...
* Open-drain output
* PWM output (hardware timer, 0.1 Hz to 32 MHz)
...
| `DIGital:PIN<n>:PWM <freq>[,<duty>]` | Start PWM output                       |
| `DIGital:PIN<n>:PWM?`         | Actual frequency and duty                    |
...
```text
Nacho.works,SCPI-TempIO v01,2038393741565017002D005E,0.3
```
...
Each DIO pin can output a hardware PWM signal, generated by a timer in the MCU. It is exact and jitter-free, and keeps running without further USB traffic.

```text
DIG:PIN0:PWM 1000          1 kHz, 50 % duty
DIG:PIN0:PWM 2.5MHZ,25     2.5 MHz, 25 % duty
DIG:PIN4:PWM 0.5,10%       0.5 Hz, 10 % duty
DIG:PIN0:PWM?              actual frequency,duty, e.g. 1000,50
DIG:PIN0:MODE OUT          stop PWM, back to normal output
```

* Frequency: 0.1 Hz to 32 MHz, as a plain number in Hz or with `HZ`, `KHZ` or `MHZ` (SCPI `MHZ` is megahertz).
* Duty: 0 to 100 %, default 50 %.
* `DIG:PIN<n>:MODE?` returns `PWM` while PWM is running. `DIG:PIN<n>:MODE IN|OUT|OD` or `*RST` stops it.
* The timer rounds to the nearest reachable setting; `DIG:PIN<n>:PWM?` returns what is actually generated. At high frequencies the duty resolution gets coarse: at 32 MHz only 0, 50 and 100 % are possible.
* Signal quality: the 330 Ω series resistor and the probe or cable capacitance round the edges. Expect a clean square wave up to roughly 5–10 MHz.

Pins that share a timer also share the frequency:

| Timer | Pins                  | Shared                                           |
| ----- | --------------------- | ------------------------------------------------ |
| TIM2  | DIO0, DIO1, DIO5      | Frequency; DIO0 and DIO5 also share the duty (same channel) |
| TIM15 | DIO2, DIO3            | Frequency                                        |
| TIM14 | DIO4                  | —                                                |
| TIM16 | DIO6                  | —                                                |
| TIM17 | DIO7                  | —                                                |

Setting a different frequency on a pin whose timer is already in use gives error `-221`; stop the other pin first or use the same frequency.
...
| -120 | Numeric data error (number expected)                  |
| -131 | Invalid suffix (unit other than HZ, KHZ, MHZ)         |
| -221 | Settings conflict (PWM timer shared with another pin) |
| -222 | Data out of range (PWM frequency or duty)             |
...
* On v0.01 boards, VTref (J1 pin 1) is routed to VBUS (5 V) instead of 3V3; see the warning at the top. Unless the board has been bodged, the ST-Link reports about 5 V target voltage and may drive SWD at that level. Bodge J1 pin 1 to 3V3 before connecting a programmer, or flash a blank board over USB DFU instead.
* Put the scope or multimeter ground on J2 pin 10 (GND), never on pin 1 (3V3). A bench scope's ground is mains earth, which the PC's USB ground is also tied to, so clipping it onto 3V3 shorts the regulator output. On one v0.01 board this destroyed U2 (AP2112K), leaving the MCU running on 5 V.
...
| 1.3      | Firmware 0.3: PWM output on all DIO pins; hardware v0.01 production warning |
